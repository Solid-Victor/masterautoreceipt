...
const SalesReceipt = () => {
  ...
  useEffect(() => {
    if (editId) {
      supabase.from('sales_receipts').select('*').eq('id', editId).single().then(({ data: record }) => {
        if (record) setData(record.data as unknown as SalesReceiptData);
      });
    }
  }, [editId]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        const { error } = await supabase.from('sales_receipts').update({
  ...
  const handleReset = () => { setData(defaultSalesReceiptData); toast.success('Form cleared'); };
