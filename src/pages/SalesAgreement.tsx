...
import { ReceiptData, defaultReceiptData } from '@/types/receipt';
...
const SalesAgreement = () => {
  ...
  useEffect(() => {
    if (editId) {
      supabase.from('sales_agreements').select('*').eq('id', editId).single().then(({ data: record }) => {
        if (record) setData(record.data as unknown as ReceiptData);
      });
    }
  }, [editId]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        const { error } = await supabase.from('sales_agreements').update({
          receipt_no: data.receiptNo,
          data: JSON.parse(JSON.stringify(data)),
        }).eq('id', editId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('sales_agreements').insert([{
          user_id: user.id,
          receipt_no: data.receiptNo,
          data: JSON.parse(JSON.stringify(data)),
        }]);
  ...
  const handleReset = () => { setData(defaultReceiptData); toast.success('Form cleared'); };
  ...
  const updateField = (field: keyof ReceiptData, value: string) => {
    setData(prev => {
      ...
      if (field === 'salePrice') {
        ...
        next.amountInWords = isNaN(num) || num === 0 ? '' : numberToWords(num);
      }
      return next;
    });
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`MasterAutoz-Agreement-${data.receiptNo}.pdf`);
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-center sm:text-left shrink-0">
              <h1 className="text-base sm:text-xl font-bold tracking-tight text-primary-foreground">
                Sales <span className="text-primary">Agreement</span>
              </h1>
            </div>
          </div>
      ...
      <div className="container mx-auto px-2 sm:px-4 py-3 sm:py-6">
        <div className={`grid grid-cols-1 ${showPreview ? 'lg:grid-cols-2' : ''} gap-4 sm:gap-6`}>
          <div className={showPreview ? 'order-2 lg:order-1' : ''}>
            <ReceiptForm data={data} onUpdate={updateField} />
          </div>
          <div className={showPreview ? 'order-1 lg:order-2 lg:sticky lg:top-16 lg:self-start' : 'order-1'}>
            ...
            {!showPreview ? (
              <button onClick={() => setShowPreview(true)} className="w-full flex flex-col items-center justify-center gap-3 py-10 rounded-lg border-2 border-dashed border-primary/30 bg-primary/5 hover:bg-primary/10 hover:border-primary/50 transition-all cursor-pointer group">
                ...
                <span className="text-sm font-semibold text-primary">Click to Preview Agreement</span>
                <span className="text-xs text-muted-foreground">See how your agreement will look before downloading</span>
              </button>
            ) : (
              <>
